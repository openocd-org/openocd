...
/* read protection magic values */

#define RDPRT			0xA5	/* stm32f1x series only */
#define RDP0			0xAA
#define RDP1			0xBB	/* or any other value except 0xAA & 0xCC */
#define RDP2			0xCC
...
COMMAND_HANDLER(stm32x_handle_lock_command)
{
	...
	if (CMD_ARGC > 1) {
		if (strcmp("permanent", CMD_ARGV[1]) == 0) {
			if (stm32x_info->default_rdp == RDPRT) {
				command_print(CMD_CTX, "permanent lock not available on stm32f1x");
				return ERROR_OK;
			}
			rdp = RDP2;
		}
	...
	/* set readout protection level */
	stm32x_info->option_bytes.rdp = rdp;
	...
	command_print(CMD_CTX, "stm32x locked%s", (rdp == RDP2) ? " permanently" : "");
...
COMMAND_HANDLER(stm32x_handle_unlock_command)
{
	...
	command_print(CMD_CTX, "stm32x unlocked.\n"
			"INFO: an stm32f1x options_load command or power cycle "
...
COMMAND_HANDLER(stm32x_handle_options_read_command)
{
	...
	uint16_t user_data = optionbyte >> ((stm32x_info->default_rdp == RDPRT) ? 10 : 16);
	...
	optionbyte >>= (stm32x_info->default_rdp == RDPRT) ? 0 : 6;
...
COMMAND_HANDLER(stm32x_handle_options_write_command)
{
	...
	command_print(CMD_CTX, "stm32x write options complete.\n"
				"INFO: an options_load command or power cycle is required "
...
static const struct command_registration stm32x_exec_command_handlers[] = {
	{
		...
		.mode = COMMAND_EXEC,
		.usage = "bank_id ('permanent')",
		.help = "Lock entire flash device against reading.",
	},
	{
		.name = "unlock",
		...
		.mode = COMMAND_EXEC,
		.usage = "bank_id",
		.help = "Unlock entire flash device for reading.",
	},
	{
		.name = "mass_erase",
	...
	},
	{
		...
		.mode = COMMAND_EXEC,
		.usage = "bank_id",
		.help = "Read and display loaded device option bytes.",
	},
	{
		.name = "options_write",
