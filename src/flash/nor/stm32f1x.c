...
static int stm32x_probe(struct flash_bank *bank)
{
	...
	for (i = 0; i < num_prot_blocks; i++) {
		bank->prot_blocks[i].offset = i * stm32x_info->ppage_size * page_size;
		bank->prot_blocks[i].size = stm32x_info->ppage_size * page_size;
		bank->prot_blocks[i].is_protected = 0;
	}
	if (num_prot_blocks < (num_pages / stm32x_info->ppage_size)) {
