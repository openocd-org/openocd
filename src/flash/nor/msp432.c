	...
	switch (msp432_bank->device_type) {
		case MSP432P401X_DEPR:
			if (0xFFFF == msp432_bank->device_id) {
				...
				command_print(cmd, "MSP432P401x pre-production device (deprecated silicon)"
					SUPPORT_MESSAGE);
			} else {
				/* Revision A or B silicon, also deprecated */
				command_print(cmd, "MSP432P401x Device Rev %c (deprecated silicon)"
					SUPPORT_MESSAGE, (char)msp432_bank->hardware_rev);
			}
			break;
		case MSP432P401X:
			command_print(cmd, "MSP432P401x Device Rev %c",
				(char)msp432_bank->hardware_rev);
			break;
		case MSP432P411X:
			command_print(cmd, "MSP432P411x Device Rev %c",
				(char)msp432_bank->hardware_rev);
			break;
		case MSP432E401Y:
			command_print(cmd, "MSP432E401Y Device");
			break;
		case MSP432E411Y:
			command_print(cmd, "MSP432E411Y Device");
			break;
		case MSP432E4X_GUESS:
