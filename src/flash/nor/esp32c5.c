...
/***************************************************************************
 *   ESP32-C5 flash driver for OpenOCD                                    *
...
/* memory map */
#define ESP32C5_DROM_LOW    0x42000000
#define ESP32C5_DROM_HIGH   0x44000000
#define ESP32C5_IROM_LOW    0x42000000
#define ESP32C5_IROM_HIGH   0x44000000

struct esp32c5_flash_bank {
	struct esp_riscv_flash_bank riscv;
};

static bool esp32c5_is_irom_address(target_addr_t addr)
{
	return addr >= ESP32C5_IROM_LOW && addr < ESP32C5_IROM_HIGH;
}

static bool esp32c5_is_drom_address(target_addr_t addr)
{
 ...
 */
FLASH_BANK_COMMAND_HANDLER(esp32c5_flash_bank_command)
{
	struct esp32c5_flash_bank *esp32c5_info;

	if (CMD_ARGC < 6)
		return ERROR_COMMAND_SYNTAX_ERROR;

	esp32c5_info = malloc(sizeof(struct esp32c5_flash_bank));
	if (!esp32c5_info)
		return ERROR_FAIL;
	int ret = esp_riscv_flash_init(&esp32c5_info->riscv,
		ESP32C5_FLASH_SECTOR_SIZE,
	...
	bank->driver_priv = esp32c5_info;
	return ERROR_OK;
}

static int esp32c5_get_info(struct flash_bank *bank, struct command_invocation *cmd)
{
	/* TODO: print some flash information */
	command_print_sameline(cmd, "Flash driver: ESP32-C5\n");
	return ERROR_OK;
}
...
const struct flash_driver esp32c5_flash = {
	.name = "esp32c5",
	.commands = esp32c5_command_handlers,
	.flash_bank_command = esp32c5_flash_bank_command,
