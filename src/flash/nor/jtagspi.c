...
struct jtagspi_flash_bank {
	...
	int probed;
	uint32_t ir;
	int addr_len;		/* address length in bytes */
};
...
static void flip_u8(const uint8_t *in, uint8_t *out, int len)
{
	for (int i = 0; i < len; i++)
...
static int jtagspi_cmd(struct flash_bank *bank, uint8_t cmd,
	uint8_t *write_buffer, int write_len, uint8_t *data_buffer, int data_len)
{
	struct jtagspi_flash_bank *info = bank->driver_priv;
...
{
	uint8_t buf;
	if (jtagspi_cmd(bank, SPIFLASH_READ_STATUS, NULL, 0, &buf, -1) == ERROR_OK) {
		*status = buf;
...
static uint8_t *fill_addr(uint32_t addr, int addr_len, uint8_t *buffer)
{
...
static int jtagspi_sector_erase(struct flash_bank *bank, int sector)
{
	...
	if (retval != ERROR_OK)
		return retval;
	retval = jtagspi_cmd(bank, info->dev.erase_cmd,
		fill_addr(bank->sectors[sector].offset, info->addr_len, addr), info->addr_len, NULL, 0);
	if (retval != ERROR_OK)
		return retval;
...
static int jtagspi_read(struct flash_bank *bank, uint8_t *buffer, uint32_t offset, uint32_t count)
{
	...
	while (count > 0) {
		...
		retval = jtagspi_cmd(bank, info->dev.read_cmd, fill_addr(offset, info->addr_len, addr),
			info->addr_len, buffer, -currsize);
		if (retval != ERROR_OK) {
...
static int jtagspi_page_write(struct flash_bank *bank, const uint8_t *buffer, uint32_t offset, uint32_t count)
{
	...
	if (retval != ERROR_OK)
		return retval;
	retval = jtagspi_cmd(bank, info->dev.pprog_cmd, fill_addr(offset, info->addr_len, addr),
		info->addr_len, (uint8_t *) buffer, count);
	if (retval != ERROR_OK)
		return retval;
