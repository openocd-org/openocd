...
static int mpc56xx_read_gpr(struct mpc5xxx_jtag *jtag_info, int reg,
		uint32_t *val)
{
	uint32_t opcode;
...
static int mpc56xx_write_gpr(struct mpc5xxx_jtag *jtag_info, int reg,
		uint32_t val)
{
	...
	opcode = 0x60000000 | (reg << 16) | (reg << 21); /* ori Rx, Rx, 0 */
	return mpc56xx_exec_inst(jtag_info, opcode, val, &val2, MPC56XX_EI_VAL);
}

int mpc56xx_read_spr(struct mpc5xxx_jtag *jtag_info, int reg,
		uint32_t *val)
{
	int retval;
	uint32_t opcode, r31;
	...
	/* Preserve R31 */
	retval = mpc56xx_read_gpr(jtag_info, MPC5XXX_REG_R31, &r31);
	if (retval)
		return retval;

	/* Transfer special purpose register to R31 */
	opcode = 0x7fe002a6 | ((reg & 0x1f) << 16) | ((reg & 0x3e0) << 6); /* mpspr r31, reg */
	retval = mpc56xx_exec_inst(jtag_info, opcode, 0, val, 0);

	/* Restore R31 */
	return mpc56xx_write_gpr(jtag_info, MPC5XXX_REG_R31, r31);
}

int mpc56xx_write_spr(struct mpc5xxx_jtag *jtag_info, int reg,
		uint32_t val)
{
	uint32_t opcode;
	...
	return mpc56xx_exec_inst(jtag_info, opcode, val, &val, MPC56XX_EI_VAL);
}

static int mpc56xx_read_cr(struct mpc5xxx_jtag *jtag_info, uint32_t *val)
{
	int retval;
	uint32_t opcode, r31;
...
static int mpc56xx_write_cr(struct mpc5xxx_jtag *jtag_info, uint32_t val)
{
	uint32_t opcode;
...
static int mpc56xx_read_ctr(struct mpc5xxx_jtag *jtag_info, uint32_t *val)
{
	int retval;
	uint32_t opcode, r31;
...
static int mpc56xx_write_ctr(struct mpc5xxx_jtag *jtag_info, uint32_t val)
{
	uint32_t opcode;
...
static int mpc56xx_read_lr(struct mpc5xxx_jtag *jtag_info, uint32_t *val)
{
	int retval;
	uint32_t opcode, r31;
...
static int mpc56xx_write_lr(struct mpc5xxx_jtag *jtag_info, uint32_t val)
{
	uint32_t opcode;
...
int mpc56xx_read_reg(struct mpc5xxx_jtag *jtag_info, int reg,
		uint32_t *val)
{
	int retval;
	...
	if (reg < 0) {
		...
		*val = 0;
		...
		retval = mpc56xx_read_gpr(jtag_info, reg, val);
		...
		*val = scr.msr;
		...
		retval = mpc56xx_read_cr(jtag_info, val);
	} else if (reg == MPC5XXX_REG_LR) {
		retval = mpc56xx_read_lr(jtag_info, val);
	} else if (reg == MPC5XXX_REG_CNT) { /* This is CTR ? */
		retval = mpc56xx_read_ctr(jtag_info, val);
	} else if (reg == MPC5XXX_REG_XER) {
		retval = mpc56xx_read_spr(jtag_info, 1, val);
...
int mpc56xx_write_reg(struct mpc5xxx_jtag *jtag_info, int reg,
		uint32_t val)
{
	int retval;
	struct mpc5xxx_cpuscr scr;
	...
	if (reg < 0) {
		...
		retval = mpc56xx_write_gpr(jtag_info, reg, val);
	} else if (reg == MPC5XXX_REG_PC) {
		retval = mpc5xxx_once_cpuscr_read(jtag_info, &scr);
		if (retval)
			return retval;
		...
		retval = mpc56xx_write_cr(jtag_info, val);
	} else if (reg == MPC5XXX_REG_LR) {
		retval = mpc56xx_write_lr(jtag_info, val);
	} else if (reg == MPC5XXX_REG_CNT) { /* This is CTR ? */
		retval = mpc56xx_write_ctr(jtag_info, val);
	} else if (reg == MPC5XXX_REG_XER) {
		retval = mpc56xx_write_spr(jtag_info, 1, val);
	} else if (reg == MPC5XXX_REG_MQ) {
		/* Ignore MQ */
		retval = ERROR_OK;
	} else {
...
int mpc56xx_jtag_read_regs(struct mpc5xxx_jtag *jtag_info, uint32_t *regs, uint32_t * saved_ctl)
{
	...
	for (i = 0; i < MPC5XXX_NUMCOREREGS - 1; i++) {
		if (!(i == MPC5XXX_REG_PC || i == MPC5XXX_REG_MSR)) {
			retval = mpc56xx_read_reg(jtag_info, i, regs + i);
			if (retval)
				return retval;
...
int mpc56xx_jtag_write_regs(struct mpc5xxx_jtag *jtag_info, uint32_t *regs)
{
	 ...
	 */
	for (i = 0; i < MPC5XXX_NUMCOREREGS - 1; i++) {
		retval = mpc56xx_write_reg(jtag_info, i, regs[i]);
		if (retval)
			return retval;
	}
