...
static int ft2232_bitbang(void *device, char *signal_name, int GETnSET, int *value)
{
	uint8_t  buf[3];
	int retval, vall = 0, valh = 0;
	unsigned int sigmask;
	uint32_t bytes_written, bytes_read;
	struct interface_signal *sig = interface_signal_find(signal_name);
...
static int ft2232_transfer(void *device, int bits, char *mosidata, char *misodata, int nLSBfirst)
{
	...
	LOG_DEBUG("(device=@%p, bits=%d, mosidata=@%p, misodata=@%p, nLSDfirst=%d) ",\
	...
	if (bits >= 8) {
		...
		if (retval < 0) {
			LOG_ERROR("write returned %d", retval);
			return ERROR_FAIL;
		}
		retval = ft2232_read((uint8_t *)buf, bytes, &bytes_read);
		if (retval < 0) {
			LOG_ERROR("read returned %d", retval);
			return ERROR_FAIL;
		}
