...
struct interface_signal *interface_signal_find(char *name)
{
	...
	while (sig) {
		if (!strncasecmp(sig->name, name, INTERFACE_SIGNAL_NAME_MAXLEN))
			return sig;
		sig = sig->next;
	}
...
int interface_signal_add(char *name, unsigned int mask)
{
	...
	struct interface_signal *newsignal, *lastsignal;
	int snlen;

	/* Check signal length (min=1, max=32 characters) */
	snlen = strnlen(name, INTERFACE_SIGNAL_NAME_MAXLEN);
	if (snlen < INTERFACE_SIGNAL_NAME_MINLEN || snlen > INTERFACE_SIGNAL_NAME_MAXLEN) {
		LOG_ERROR("Signal name too short or too long!");
		return ERROR_FAIL;
	}

	/* Check if signal name already exist and return error if so */
	if (interface_signal_find(name)) {
		LOG_ERROR("Specified signal already exist!");
		return ERROR_FAIL;
	}

	/* Allocate memory for new signal structure */
	newsignal = (struct interface_signal *)calloc(1, sizeof(struct interface_signal));
	if (!newsignal) {
...
int interface_signal_del(char *name)
{
	...
	struct interface_signal *delsig = NULL, *prevsig = NULL;

	/* look for the signal name on the list */
	delsig = interface_signal_find(name);
