 ...
 */
COMMAND_HANDLER(handle_interface_bitbang_command)
{
	LOG_DEBUG(" ");	/* space to avoid "error: zero-length gnu_printf format string" */
	...
	static struct interface_signal *sig;
	static unsigned int pn;
	static int retval;
	static char *mark, *signame, *sigval, pcmd[BITBANG_PARAM_CMD_MAX_LEN];

	/* Iterate through list of command parameters */
	for (pn = 0; pn < CMD_ARGC; pn++) {
		/* Make a local copy of parameter command to work on */
		if (!strncpy(pcmd, CMD_ARGV[pn], BITBANG_PARAM_CMD_MAX_LEN)) {
		...
		if (!mark) {
			...
			sig = interface_signal_find(pcmd);
			if (!sig) {
			...
			signame = strtok(pcmd, "=");
			sig = interface_signal_find(signame);
			if (!sig) {
				LOG_ERROR("Unknown signal specified!");
				return ERROR_FAIL;
			}
...
static const struct command_registration interface_bitbang_commands[] = {
	{
		.name = "interface_bitbang",
		.handler = handle_interface_bitbang_command,
		.mode = COMMAND_EXEC,
...
int interface_bitbang_register_commands(struct command_context *ctx)
{
	return register_commands(ctx, NULL, interface_bitbang_commands);
};
